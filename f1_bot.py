...
🎥 Yayım linklərini görün""",
    ...
    "help_title": "🏎️ F1 Canlı Bot Köməyi",
...
• /menu - Düymə menyusunu göstər""",
    "help_usage": """*Məlimat İstifadəsi:* ~2-5 KB hər sorğu 📱
...
• Formula Timer (canlı vaxt scraper)""",
    ...
    "error_fetching_weather": "❌ Hava məlimatları alınarkən xəta: {}",
...
try:
    ...
    logging.warning(f"Fallback scraper not available: {e}")
    ...
    try:
        if query.data == "standings":
            ...
            if not check_active_f1_session():
                ...
                try:
                    # Initial fetch using fallback scraper
...
def home():
    ...
    return {
        "status": "F1 Telegram Bot (Railway Deployment) is running!",
        ...
        "features": {
            "containerized": True,
            "webhook_mode": True,
            "optimized_scraping": OPTIMIZED_SCRAPER_AVAILABLE or SCRAPER_AVAILABLE,
...
if __name__ != "__main__":
    ...
    logger.info("Starting F1 Bot in production mode...")
    setup_bot()
